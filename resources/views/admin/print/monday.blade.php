        ...
        @for ($i = 1; $i <= config('constant.totalNumberOfSeat'); $i++)     
            <tr style='background-color:{{$i % 2 ? "#FCF3CF" : "#ffffff"}}'>
                ...
                @if(in_array($i, $bookingSeatsMonday)) 
                    <td style="border-style: solid; border-color: black; border-width: 1px;">{{ $bookingsMonday[array_search($i, $bookingSeatsMonday)]->employee->empId }}</td>
                    <td style="border-style: solid; border-color: black; border-width: 1px;">{{ $bookingsMonday[array_search($i, $bookingSeatsMonday)]->employee->name }}</td>
                    <td style="border-style: solid; border-color: black; border-width: 1px;">{{ $bookingsMonday[array_search($i, $bookingSeatsMonday)]->employee->dept }}</td>
                    <td style="border-style: solid; border-color: black; border-width: 1px;">{{ $bookingsMonday[array_search($i, $bookingSeatsMonday)]->pword }}</td>
                    <td style="border-style: solid; border-color: black; border-width: 1px;">{{ $bookingsMonday[array_search($i, $bookingSeatsMonday)]->destination }}</td>
                    <td style="border-style: solid; border-color: black; border-width: 1px;">__________</td>
                @else
